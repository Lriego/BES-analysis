...
usage() {
	...
	echo -e "	5) 'align' aligns them to a reference genome with one of the following methods:"
	echo -e "\t\t-BLAST\n\t\t-MEGA (megablast)\n\t\t-BLAT\n\t\t-NUC (nucmer)\n\t\t-BOW (bowtie2)"
	...
	align | all.4)
		...
		case "$METHOD" in
			...
			BOW | all.5)
				...
				bowtie2 -f -x $(dirname $GENOME)/bt2_indexes/$(basename $GENOME) -U "$INPUT" -S "$OUTPUT"/"$(basename "$INPUT" .fasta)_bow.out" --no-unal
			...
			*)
				echo "Error, -m (method for aligment) is a mandatory option when align or all stages are specified.  Please use the -h option to view the help for valid options"
				exit 0
			;;
		esac
	;;
